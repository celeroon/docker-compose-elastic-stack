...
if [ ! -f "$FLAG_FILE_SYSTEM_PACKAGE" ]; then
    ...
    curl --cacert /certs/elasticsearch-ca.pem -s -u "elastic:${ELASTIC_PASSWORD}" \
        ...
        -d '{"name":"System","namespace":"default","policy_id":"windows-policy", "package":{"name": "system", "version":"1.55.2"}}' > /dev/null
...
#         -XPOST -H "kbn-xsrf: kibana" -H "Content-type: application/json" \
#         "https://kibana:5601/api/fleet/package_policies" \
#         -d '{"name":"EDR","description":"","namespace":"default","enabled":true,"package":{"name":"endpoint","title":"Elastic Defend","version":"8.13.0"},"inputs":[{"enabled":true,"streams":[],"type":"ENDPOINT_INTEGRATION_CONFIG","config":{"_config":{"value":{"type":"endpoint","endpointConfig":{"preset":"DataCollection"}}},"policy":{"value":{"meta":{"license":"basic","cluster_name":"'${CLUSTER_NAME}'","cloud":false,"serverless":false},"global_manifest_version":"latest","windows":{"events":{"credential_access":true,"dll_and_driver_load":true,"dns":true,"file":true,"network":true,"process":true,"registry":true,"security":true},"malware":{"mode":"off","blocklist":false},"ransomware":{"mode":"off","supported":false},"memory_protection":{"mode":"off","supported":false},"behavior_protection":{"mode":"off","reputation_service":false,"supported":false},"popup":{"malware":{"message":"","enabled":true},"ransomware":{"message":"","enabled":false},"memory_protection":{"message":"","enabled":false},"behavior_protection":{"message":"","enabled":false}},"logging":{"file":"info"},"antivirus_registration":{"enabled":false},"attack_surface_reduction":{"credential_hardening":{"enabled":false}}},"mac":{"events":{"process":true,"file":true,"network":true},"malware":{"mode":"off","blocklist":false},"behavior_protection":{"mode":"off","reputation_service":false,"supported":false},"memory_protection":{"mode":"off","supported":false},"popup":{"malware":{"message":"","enabled":true},"behavior_protection":{"message":"","enabled":false},"memory_protection":{"message":"","enabled":false}},"logging":{"file":"info"},"advanced":{"capture_env_vars":"DYLD_INSERT_LIBRARIES,DYLD_FRAMEWORK_PATH,DYLD_LIBRARY_PATH,LD_PRELOAD"}},"linux":{"events":{"process":true,"file":true,"network":true,"session_data":false,"tty_io":false},"malware":{"mode":"off","blocklist":false},"behavior_protection":{"mode":"off","reputation_service":false,"supported":false},"memory_protection":{"mode":"off","supported":false},"popup":{"malware":{"message":"","enabled":true},"behavior_protection":{"message":"","enabled":false},"memory_protection":{"message":"","enabled":false}},"logging":{"file":"info"},"advanced":{"capture_env_vars":"LD_PRELOAD,LD_LIBRARY_PATH"}}}}}}]}' > /dev/null
